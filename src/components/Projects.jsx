      ...
      <div className="flex flex-col items-center">
        ...
        <p className="text-white/90 text-xs sm:text-sm mt-3 font-light text-center">
          {isMobile ? (
            ...
            <span>Click and drag right to watch, left to skip<br/>Or use arrow keys ← →</span>
          )}
        </p>
      </div>
    </motion.div>
        ...
        <div className={`relative w-full transition-all duration-300 mt-10 ${isFocused ? 'scale-105' : 'scale-100'}`}>
          ...
          <div className={`relative rounded-full overflow-hidden bg-gradient-to-r from-violet-600 to-blue-600 p-[1px] transition-all duration-300 ${isFocused ? 'shadow-lg shadow-violet-500/20' : ''}`}>
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
              <svg 
                className={`w-5 h-5 transition-colors duration-300 ${isFocused ? 'text-violet-400' : 'text-gray-400'}`} 
                fill="none" 
                stroke="currentColor" 
                viewBox="0 0 24 24"
              >
          ...
          <div className="bg-gray-900 rounded-lg overflow-hidden relative">
            <div className="flex">
              <button
                ...
                className={`flex items-center justify-center py-3 px-3 sm:px-4 text-xs sm:text-sm md:text-base relative z-20 w-1/2 transition-colors duration-300 ${!isGridView ? 'text-white font-medium' : 'text-gray-400'}`}
              >
                <svg className="w-4 h-4 mr-1 sm:mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
